      ...
      extR(M3D::Identity())
{
  ...
  running_.store(true);
  imu_thread_ = std::thread(&LIVMapper::imu_prop_loop, this);
}
...
void LIVMapper::initializeSubscribersAndPublishers(ros::NodeHandle &nh, image_transport::ImageTransport &it) 
{
  ...
  pubLaserCloudFullRes = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/cloud_registered", 100);
  pubNormal = nh.advertise<visualization_msgs::MarkerArray>("/LIVO/visualization_marker", 100);
  pubSubVisualMap = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/cloud_visual_sub_map_before", 100);
  pubLaserCloudEffect = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/cloud_effected", 100);
  pubLaserCloudMap = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/Laser_map", 100);
  pubOdomAftMapped = nh.advertise<nav_msgs::Odometry>("/LIVO/odometry", 10);
  pubPath = nh.advertise<nav_msgs::Path>("/LIVO/path", 10);
  plane_pub = nh.advertise<visualization_msgs::Marker>("/LIVO/planner_normal", 1);
  voxel_pub = nh.advertise<visualization_msgs::MarkerArray>("/LIVO/voxels", 1);
  pubLaserCloudDyn = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/dyn_obj", 100);
  pubLaserCloudDynRmed = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/dyn_obj_removed", 100);
  pubLaserCloudDynDbg = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/dyn_obj_dbg_hist", 100);
  mavros_pose_publisher = nh.advertise<geometry_msgs::PoseStamped>("/LIVO/mavros/vision_pose/pose", 10);
  pubImage = it.advertise("/LIVO/rgb_img", 1);
  pubImuPropOdom = nh.advertise<nav_msgs::Odometry>("/LIVO2/imu_propagate_odom", 10000);
  // imu_prop_timer = nh.createTimer(ros::Duration(0.005), &LIVMapper::imu_prop_callback, this);
  voxelmap_manager->voxel_map_pub_= nh.advertise<visualization_msgs::MarkerArray>("/LIVO/planes", 10000);
  pubCurrentCloud = nh.advertise<sensor_msgs::PointCloud2>("/LIVO/current_cloud", 1);
}
...
{    
  ...
  // // add publisher
  // sensor_msgs::PointCloud2 current_local_msg;
  // pcl::toROSMsg(*laserCloudFullRes, current_local_msg);
  // current_local_msg.header.stamp = ros::Time::now();
  // current_local_msg.header.frame_id = "aft_mapped";
  // pubCurrentLocalCloud.publish(current_local_msg);
...
{
  // std::cout << "imu_prop_callback " << std::endl;
  // ROS_INFO("\033[34mROS Time: %d.%09d\033[0m", ros::Time::now().sec, ros::Time::now().nsec);
  ros::Time a = ros::Time::now();
  // std::cout << "1: " << ros::Time::now().toSec() << std::endl;
  if (p_imu->imu_need_init || !new_imu || !ekf_finish_once) {
    // std::cout << "pass " << p_imu->imu_need_init <<  !new_imu << !ekf_finish_once << std::endl;
    return;  
  }
  ...
  ros::Duration dt = ros::Time::now() - a;  
  // std::cout << "2: " << dt.toSec() << std::endl;
}
...
void LIVMapper::livox_pcl_cbk(const livox_ros_driver2::CustomMsg::ConstPtr &msg_in)
{
  ...
  pcl::toROSMsg(*ptr, current_msg);
  current_msg.header.stamp = ros::Time().fromSec(cur_head_time);
  current_msg.header.frame_id = "aft_mapped";
  pubCurrentCloud.publish(current_msg);
